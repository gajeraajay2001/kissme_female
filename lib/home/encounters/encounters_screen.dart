...
class _EncountersScreenState extends State<EncountersScreen> {
  ...
  _save() async {
    ...
    if (parseResponse.success) {
      ...
      context
          ...
          .updateUser(parseResponse.results!.first!);
  ...
  Future<List<dynamic>?> loadUser() async {
    ...
    if (encounterResponse.success) {
      ...
      // users.whereValueExists(
      //     UserModel.keyBirthday, true); // Only show users with birthday
  ...
  //                       enlargeCenterPage:
  //                       QuickHelp.getPhotosCounter(selectedUser).length > 1
  //                           ? true
  //                           : false,
  //                       viewportFraction: 1,
  ...
  Widget getBody() {
    ...
    return Container(
      ...
      child: GridView.count(
        ...
        mainAxisSpacing: 8,
        crossAxisSpacing: 8,
        children: List.generate(globalUsers.length, (index) {
          UserModel selectedUser = globalUsers[index];
                        ...
                        child: Align(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              color:
                                  QuickHelp.isUserOnlineStatus(selectedUser) ==
                        ...
                        CarouselSlider.builder(
                            options: CarouselOptions(
                                ...
                                autoPlayInterval: Duration(
                                    seconds: Setup.photoSliderDuration),
                                enlargeCenterPage:
                                    QuickHelp.getPhotosCounter(selectedUser)
                                                .length >
                                            1
                            ...
                            carouselController: _controller,
                            itemCount:
                                QuickHelp.getPhotosCounter(globalUsers[index]!)
                                    .length,
                            itemBuilder: (BuildContext context, int itemIndex,
                                int pageViewIndex) {
                              return ContainerCorner(
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: Column(
                            children: [
                              info(
                                  "assets/svg/prof.svg", getWork(selectedUser)),
                              info("assets/svg/school.svg",
                                  selectedUser.getSchool!),
                              info(
                                  "assets/svg/sex.svg",
                                  QuickHelp.getSexualityListWithName(
                                      selectedUser)),
                              info("assets/svg/country.svg",
                                  selectedUser.getLocation!),
                            ],
                          ),
                        ),
                        Padding(
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          GestureDetector(
            onTap: () => checkPermission(true, model: userModel),
            child: Container(
          ...
          //
          //     GestureDetector(
          //       onTap: () => _showVideoAndVoiceCallsButtons(model: userModel),
          //       child: Container(
          //         width: 40,
          //         height: 40,
  ...
  _showVideoAndVoiceCallsButtons({required UserModel model}) {
    showDialog(
        context: context,
                      ...
                      children: [
                        BounceInUp(
                          child: ContainerCorner(
                            ...
                            onTap: () => checkPermission(false, model: model),
                          ),
                        ),
                        BounceInRight(
                          child: ContainerCorner(
                            ...
                            onTap: () => checkPermission(true, model: model),
                          ),
                        )
                      ],
                    ),
  ...
  checkPermission(bool isVideoCall,
      {required UserModel model, bool isDirectCall = true}) async {
    if (!isDirectCall) {
      QuickHelp.goBackToPreviousPage(context);
    }
    if (await Permission.camera.isGranted &&
        await Permission.microphone.isGranted) {
      startCall(isVideoCall, model: model);
  ...
  startCall(bool isVideoCall, {required UserModel model}) {
    if (isVideoCall) {
