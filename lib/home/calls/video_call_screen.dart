...
class _State extends State<VideoCallScreen> {
  ...
  void initState() {
    ...
    context.read<CallsProvider>().setCallRunning(false);
    Wakelock.enable();
  ...
  _initEngine() async {
    _engine = await RtcEngine.create(Config.agoraAppId).catchError((error) {
      print(error);
    });
    this._addListeners();
  ...
  _addListeners() {
    _engine!.setEventHandler(RtcEngineEventHandler(
      joinChannelSuccess: (channel, uid, elapsed) {
        ...
        context.read<CallsProvider>().setCallRunning(true);
    ...
    if (widget.currentUser!.getCredits! >=
      ...
      widget.currentUser!.save().then((value) {
        ...
        QuickCloudCode.sendGift(
            authorId: widget.mUser!.objectId!,
  ...
  endCallBtnCaller(String endReason) {
    ...
    Future.delayed(Duration(seconds: 1), () {
      print(context.read<CallsProvider>().isStreamingOn);
      if (context.read<CallsProvider>().isStreamingOn) {
