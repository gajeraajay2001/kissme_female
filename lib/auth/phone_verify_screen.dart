...
class PhoneVerifyScreen extends StatefulWidget {
  ...
  PhoneVerifyScreen({this.currentUser});
...
class _PhoneVerifyScreenState extends State<PhoneVerifyScreen> {
  ...
  Future<void> _checkUserAccount() async {
    ...
    ParseResponse apiResponse = await widget.currentUser!.save();

    if (apiResponse.success) {
      UserModel userModel = apiResponse.results!.first;
      QuickHelp.goBackToPreviousPage(context, result: userModel);
    } else {
  ...
  Widget body() {
    return Form(
      key: _formKey,
      child: Column(
        mainAxisAlignment: QuickHelp.isMobile()
  ...
  Widget phoneNumberInput() {
    return Padding(
      ...
      child: Column(
        children: [
          TextWithTap(
            "payment_screen.verify_phone_".tr(),
            fontSize: 25,
            marginBottom: 100,
